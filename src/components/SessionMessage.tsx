...
export const SessionMessageCustom: FC<Props> = ({ conversation, isLast }) => {
  // Deduplicate sources if they exist
  const uniqueSources = useMemo(() => {
    if (!conversation.sources || !Array.isArray(conversation.sources)) {
      return [];
    }
    
    const sourceMap = new Map();
    conversation.sources.forEach(source => {
      const uniqueKey = source.title || source.url || JSON.stringify(source);
      sourceMap.set(uniqueKey, source);
    });
          ...
          <div className='flex-1'>
            {conversation.response === undefined ? (
              ...
              <>
                ...
                {/* Use the deduplicated sources here */}
                <MessageSources sources={uniqueSources} />
              </>
            )}
          </div>
