...
export const GraphVisualization: FC<Props> = ({ kg, id }) => {
  ...
  const [selectedLinkTypes, setSelectedLinkTypes] = useState<Set<string>>(
    new Set()
  );
  ...
  useEffect(() => {
    ...
    kg.nodes.forEach((node: any) => {
      ...
      nodeCategoryCounts.set(
        category,
        (nodeCategoryCounts.get(category) || 0) + 1
      );
    ...
    const newNodeTypes = Array.from(nodeCategories.entries()).map(
      ([category, color]) =>
        ({
          category,
          color,
    ...
    kg.links.forEach((link: any) => {
      ...
      linkCategoryCounts.set(
        predicate,
        (linkCategoryCounts.get(predicate) || 0) + 1
      );
    ...
    const newLinkTypes = Array.from(linkPredicates.entries()).map(
      ([predicate, color]) =>
        ({
          predicate,
          color,
  ...
  const filteredGraphData = useMemo(() => {
    ...
    const visibleNodeIds = new Set(visibleNodes.map((n: any) => String(n.id)));

    // 2. Filter links to include only:
    //    - Selected link types
    //    - Links where BOTH source and target are visible
    const filteredLinks = kg.links.filter(
      (link: any) =>
  ...
  const generateDescription = (description) => {
    return description
      ? description.length > 300
        ? description.toLowerCase().slice(0, 300) + "..."
  ...
  const LinkLabel = ({ link }) => (
    <div>
      ...
      <div>
        <em>
          <strong>is {link.predicate}</strong>
        </em>
      </div>
      <br />
