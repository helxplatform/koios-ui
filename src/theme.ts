...
export const chatTheme: ChatTheme =  {
  ...
  sessions: {
    base: 'overflow-auto',
    console:
      'min-w-[150px] w-[30%] max-w-[300px] bg-[#F2F3F7] p-5 rounded-3xl',
    companion: 'w-full h-full',
    group:
      'text-xs dart:text-gray-400 text-gray-700 mt-4 hover:bg-transparent mb-1',
    create: 'relative mb-4 rounded-[10px] text-white bg-[#1a568c] hover:bg-[#41ABF5] transition-colors',
    session: {
      base: [
  ...
  messages: {
    ...
    title: ['text-base font-bold text-gray-500'].join(
      ' '
    ),
    ...
    header: 'flex justify-between items-center gap-2 relative top-2',
    showMore: 'mb-4',
    message: {
      ...
      response: ['relative data-[compact=false]:px-4 text-gray-900'].join(' '),
      overlay: `overflow-y-hidden max-h-[350px] after:content-[''] after:absolute after:inset-x-0 after:bottom-0 after:h-16 after:bg-gradient-to-b after:from-transparent after:to-gray-200`,
      ...
      files: {
        ...
        file: {
          ...
          name: ['text-sm text-gray-500'].join(' ')
        }
      },
      sources: {
      ...
      markdown: {
        ...
        toolbar: 'text-xs flex items-center justify-between px-2 py-1 rounded-t sticky top-0 backdrop-blur-md bg-gray-200 ',
        li: 'mb-2 ml-6',
        ul: 'mb-4 list-disc',
  ...
  input: {
    ...
    input: [
      'w-full border rounded-3xl px-3 py-2 pr-16 text-gray-500 border-gray-200 hover:bg-[#41ABF5]/10 hover:border-[#41ABF5] after:hidden after:!mx-10 bg-white [&>textarea]:w-full [&>textarea]:flex-none',
    ].join(' '),
    actions: {
      base: 'absolute flex gap-6 items-center right-5 inset-y-1/2 -translate-y-1/2 z-10',
      send: [
        'px-3 py-3 hover:bg-[#41ABF5] rounded-full bg-[#1a568c] text-white transition-colors',
      ].join(' '),
